			...
			for (var i:int = 0; i < numSlot; i++)
			{
				...
				if (gift != null) 
				{
					...
					if (gift.ClassName == "GiftNormal")
					{
						...
						(itGift as ItemNormalGift).xNum = -20;
						(itGift as ItemNormalGift).yNum = 50;
					}
					...
					if (gift.ClassName == "GiftSpecial") {
						...
						if (top == 1)
						{
							(itGift as ItemSpecialGift).addNum( 10, 3, 15, 0xffff00);
							(itGift as ItemSpecialGift).addName( -17, 70);
						}
						else {
							(itGift as ItemSpecialGift).addName( -17, 67);
						}
					}
